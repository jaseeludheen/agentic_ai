...
def get_order_details(order_id):
    try:
        ...
        return {
            ...
            "ordered_on": order.created_at.strftime("%d %b %Y"), # 01 JAN 2026
            "days_since_ordered": (timezone.now() - order.created_at).days,   # 4 days
        }
...
def get_refund_history(user_id):
    ...
    for refund in refunds:
        history.append({
            ...
            "requested_on": refund.created_at.strftime("%d %b %Y"),  # 01 JAN 2026
...
def check_delivery_status(tracking_number, carrier):
    default_response = {
        "status": "Delivery information not available",
